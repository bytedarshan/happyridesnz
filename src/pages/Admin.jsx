    ...
    <div className="admin-section">
      ...
      <div className="category-tabs" style={{ display: 'flex', gap: '1rem', overflowX: 'auto', paddingBottom: '1rem', marginBottom: '2rem' }}>
        {Object.keys(siteData.packages).map(cat => (
          <button 
            key={cat}
            className={`pop-tag ${selectedCategory === cat ? 'active' : ''}`}
        ...
        {siteData.packages[selectedCategory].map(pkg => (
          <div key={pkg.id} className="glass-panel" style={{ padding: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: '1rem' }}>
              <button className="btn-outline" onClick={() => setEditingItem({ ...pkg, type: 'package', mode: 'edit', category: selectedCategory })}>
                <Edit3 size={18} />
              </button>
              <button className="btn-outline" style={{ color: '#EF4444' }} onClick={() => removePackage(selectedCategory, pkg.id)}>
                <Trash2 size={18} />
              </button>
            </div>
          </div>
  ...
  const renderTestimonials = () => (
    <div className="admin-section">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '3rem' }}>
        ...
        <button 
          className="btn-primary-glass" 
          onClick={() => setEditingItem({ type: 'testimonial', mode: 'add' })}
          style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}
        >
      ...
      <div className="admin-list" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        {siteData.testimonials.map(test => (
          <div key={test.id} className="glass-panel" style={{ padding: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <button className="btn-outline" style={{ color: '#EF4444' }} onClick={() => removeTestimonial(test.id)}>
              <Trash2 size={18} />
            </button>
          </div>
        ))}
